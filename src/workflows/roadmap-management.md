...
### Adapted from GSD
- add-phase: Phase number calculation, ROADMAP.md update, directory creation
- remove-phase: Status validation, confirmation flow, renumbering logic
- Both: STATE.md decision logging, milestone total updates

### PAUL Innovations Beyond GSD
- **Combined workflow:** Both operations in one file for cohesion
- **Clearer validation rules:** Explicit section on what's allowed
- **Directory renumbering:** Automatic renumbering of subsequent phase directories
- **Decision logging:** Both operations add to STATE.md decisions table
- **No insert-phase:** Decimal phases (e.g., 5.1) handle interruptions instead

### Design Decision: No insert-phase
GSD has insert-phase for adding phases between existing ones. PAUL uses decimal phases instead:
- If urgent work appears between Phase 5 and 6, create Phase 5.1
- Simpler: no renumbering, clear that it's an interruption
- PAUL Rule 10: "Urgent work uses decimal phases (2.1, 2.2). Integers = planned, decimals = interruptions."
